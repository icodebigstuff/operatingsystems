...
int main(int argc, char *argv[]) {
  ...
  struct addrinfo hints = {                             // hints allow a specific kind of socket
    .ai_family =   AF_INET6,                            // to be set up for the server; default
    .ai_socktype = SOCK_STREAM,                         // to using ipv6 addresses with backwards
    .ai_flags =    AI_PASSIVE,                          // ipv4 compatibility
  };
  // NOTE: using AF_INET6 allows either IPv4 or IPv6 connections; an
  // alternative is AF_UNSPEC which may return multiple addrinfo
  // structs but seems to prefer IPv4 only which precludes use of IPv6
  // addresses. The following command line args allow tinkering with this.
  if(argc > 1 && strcmp(argv[1],"-ipv4")==0){           // use ipv4 addresses only
    hints.ai_family = AF_INET;                         
  }
  if(argc > 1 && strcmp(argv[1],"-unspec")==0){         // use "unspecified" address types
    hints.ai_family = AF_UNSPEC;
  }


  struct addrinfo *serv_addr;                           // address of the server
  ret = getaddrinfo(NULL, PORT, &hints, &serv_addr);    // find server (my) address, note NULL for 1st arg
  assert(ret == 0);
  ...
  while(1){                                             // server listens forever
    ...
    // POLL FIRST
    struct pollfd pfds[1] = {{.fd=listen_fd, .events=POLLIN}};
    printf("poll() listen_fd %d\n",listen_fd);
    int ret = poll(pfds, 1, -1);
    printf("poll() returned %d\n",ret);    

    printf("pfds[0].revents & POLLIN == %d\n",
           pfds[0].revents & POLLIN);

    if(pfds[0].revents & POLLIN){
      printf("connection ready, accept will return immediately\n");
    }

    // POLL COMPLETED SO SHOULD BE 
